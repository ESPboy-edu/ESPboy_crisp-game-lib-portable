...
#define MAX_HIT_BOX_COUNT 128//256
/// \endcond
...
// Drawing
/// \cond
#define MAX_DRAWING_HIT_BOXES_COUNT 32//64
/// \endcond
...
static void drawCharacter(int index, float x, float y, bool _hasCollision, bool isText, Collision *hitCollision) {
  static CharacterPattern cp;
  
  if ((isText && (index < '!' || index > '~')) || (!isText && (index < 'a' || index > 'z'))) {
    return;
  }
  
   setColorGrid(isText ? textPatterns[index - '!'] : characters[index - 'a'], cp.grid, color, &cp.hitBox);
...
#define MAX_SCORE_BOARD_COUNT 8//16
/// \endcond
