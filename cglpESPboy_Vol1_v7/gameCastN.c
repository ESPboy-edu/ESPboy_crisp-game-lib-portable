...
static void update() {
  if (!ticks) {
    fishes = (Fish*)sharedGameRAM;
    nodes = (Node *)(sharedGameRAM + (sizeof(Fish) * CAST_N_MAX_FISH_COUNT));
    ...
    nodeState = 0;
    throwPower = 0;
    INIT_UNALIVED_ARRAY_PTR(fishes,CAST_N_MAX_FISH_COUNT);
    fishIndex = 0;
    nextFishTicks = 0;
    nextRedFishCount = 1;
    waterY = 40;
  ...
  if (nodeState == 2) {
    FOR_EACH_PTR(nodes, CAST_N_NODES_COUNT, i) {
  ...
  COUNT_IS_ALIVE_PTR(fishes, CAST_N_MAX_FISH_COUNT, fc);
  if (fc == 0) {
    nextFishTicks = 0;
  }
  nextFishTicks--;
  ...
  FOR_EACH_PTR(fishes, CAST_N_MAX_FISH_COUNT, i) {
