...
static void updateLanes() {
  FOR_EACH_PTR(lanes, FROOOOG_MAX_LANE_COUNT, i) {
  ...
  FOR_EACH_PTR(cars, FROOOOG_MAX_CAR_COUNT, i) {
...
static void update() {
  if (!ticks) {
    cars = (Car*)sharedGameRAM; 
    lanes = (Lane*)(sharedGameRAM + (sizeof(Car) * FROOOOG_MAX_CAR_COUNT));
    INIT_UNALIVED_ARRAY_PTR(lanes, FROOOOG_MAX_LANE_COUNT);
    laneIndex = 0;
    INIT_UNALIVED_ARRAY_PTR(cars, FROOOOG_MAX_CAR_COUNT);
    carIndex = 0;
    nextLaneY = 90;
