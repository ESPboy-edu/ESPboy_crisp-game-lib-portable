...
static void initPlayers() {
  FOR_EACH_PTR(players, SURVIVOR_PLAYER_COUNT, i) {
...
static void addPlayers() {
  play(POWER_UP);
  FOR_EACH_PTR(players, SURVIVOR_PLAYER_COUNT, i) { addPlayer(); }
}

static void addDownedPlayer(Vector pos, float vx, float vy) {
  FOR_EACH_PTR(downedPlayers, SURVIVOR_PLAYER_COUNT, i) {
...
static void update() {
  if (!ticks) {
    players = (Player *) sharedGameRAM;
    downedPlayers = (DownedPlayer *) (sharedGameRAM + (sizeof(Player) * SURVIVOR_PLAYER_COUNT));
    initPlayers();
  ...
  FOR_EACH_PTR(players, SURVIVOR_PLAYER_COUNT, i) {
    ...
    p->ticks++;
    if (p->underFoot == NULL) {
      FOR_EACH_PTR(players, SURVIVOR_PLAYER_COUNT, j) {
  ...
  COUNT_IS_ALIVE_PTR(players, SURVIVOR_PLAYER_COUNT, playerCount);
  if (playerCount == 0) {
    play(RANDOM);
    gameOver();
  }
