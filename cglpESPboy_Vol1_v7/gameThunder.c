...
#define THUNDER_MAX_LINE_COUNT 100
...
static void update() {
  if (!ticks) {
    lines = (Line *)sharedGameRAM;
    stars = (Star *)(sharedGameRAM + (sizeof(Line) * THUNDER_MAX_LINE_COUNT));
    for (uint8_t i = 1; i < THUNDER_MAX_LINE_COUNT; i++) {
      lines[i].prevLine = &lines[i - 1];
    }
    INIT_UNALIVED_ARRAY_PTR(lines, THUNDER_MAX_LINE_COUNT);
    lineIndex = 0;
    INIT_UNALIVED_ARRAY_PTR(stars, THUNDER_MAX_STAR_COUNT);
    starIndex = 0;
    activeTicks = -1;
    player.x = 40;
    player.vx = 1;
  ...
  COUNT_IS_ALIVE_PTR(lines, THUNDER_MAX_LINE_COUNT,lc);
  if (lc == 0) {
  ...
  rect(0, 90, 100, 10);
  activeTicks--;
  FOR_EACH_PTR(lines, THUNDER_MAX_LINE_COUNT, i) {
  ...
  color = YELLOW;
  FOR_EACH_PTR(stars, THUNDER_MAX_STAR_COUNT, i) {
