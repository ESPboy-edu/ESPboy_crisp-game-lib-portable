...
static void update() {
  if (!ticks) {
    panels = (Panel *)sharedGameRAM;
    coins = (Coin *)(sharedGameRAM + (sizeof(Panel) * LADDER_DROP_MAX_PANEL_COUNT));
    INIT_UNALIVED_ARRAY_PTR(panels, LADDER_DROP_MAX_PANEL_COUNT);
    ...
    player.vx = 1;
    player.state = 0;
    INIT_UNALIVED_ARRAY_PTR(coins, LADDER_DROP_MAX_COIN_COUNT);
    coinIndex = 0;
    scr = totalScr = 0;
    multiplier = 1;
    lockCount = 0;
  ...
  color = BLACK;
  float minY = 99;
  FOR_EACH_PTR(panels, LADDER_DROP_MAX_PANEL_COUNT, i) {
  ...
  color = BLACK;
  FOR_EACH_PTR(coins, LADDER_DROP_MAX_COIN_COUNT,i) {
