...
static void update() {
  if (!ticks) {
    spikes = (Spike *)sharedGameRAM;
    bonuses = (Bonus *)(sharedGameRAM + (sizeof(Spike) * RWHEEL_SPIKE_COUNT));
    bars = (Bar *)(sharedGameRAM + (sizeof(Spike)*RWHEEL_SPIKE_COUNT) + (sizeof(Bonus) * RWHEEL_MAX_BONUS_COUNT));
    FOR_EACH_PTR(spikes, RWHEEL_SPIKE_COUNT, i) {
      ASSIGN_ARRAY_ITEM(spikes, i, Spike, s);
      s->height = 0;
      s->isHit = false;
    }
    angleOfs = 0;
    INIT_UNALIVED_ARRAY_PTR(bonuses, RWHEEL_MAX_BONUS_COUNT);
    bonusIndex = 0;
    player.y = 0;
    player.vy = 0;
    INIT_UNALIVED_ARRAY_PTR(bars, RWHEEL_MAX_BAR_COUNT);
    barIndex = 0;
    multiplier = 1;
    validSpikeCount = 0;
  }
  ...
  float va = 0.03 * sd;
  color = YELLOW;
  FOR_EACH_PTR(bonuses, RWHEEL_MAX_BONUS_COUNT,i) {
  ...
  FOR_EACH_PTR(bars, RWHEEL_MAX_BAR_COUNT,i) {
