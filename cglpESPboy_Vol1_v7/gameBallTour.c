...
static void update() {
  if (!ticks) {
    ...
    balls = (Ball*)(sharedGameRAM + (sizeof(Spike) * BALL_TOUR_MAX_SPIKE_COUNT));
    ...
    player.vx = 0;
    player.ticks = 0;
    INIT_UNALIVED_ARRAY_PTR(spikes, BALL_TOUR_MAX_SPIKE_COUNT);
    spikeIndex = 0;
    nextSpikeDist = 0;
    INIT_UNALIVED_ARRAY_PTR(balls, BALL_TOUR_MAX_BALL_COUNT);
    ballIndex = 0;
    nextBallDist = 9;
    multiplier = 1;
  }
  ...
  color = BLACK;
  FOR_EACH_PTR(spikes, BALL_TOUR_MAX_SPIKE_COUNT, i) {
  ...
  color = GREEN;
  FOR_EACH_PTR(balls, BALL_TOUR_MAX_BALL_COUNT,i) {
