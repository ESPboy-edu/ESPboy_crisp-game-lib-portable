...
#define MAX_CHARACTER_PATTERN_COUNT 150
#define ASCII_CHARACTER_COUNT 127
#define MAX_CACHED_CHARACTER_PATTERN_COUNT 0//128
...
#define FOR_EACH_PTR(array, max_count, index) \
  for (int index = 0; index < max_count; index++)

#define COUNT_IS_ALIVE_PTR(array, max_count, counter) \
  int counter = 0;                                    \
  do {                                                \
...
#define INIT_UNALIVED_ARRAY_PTR(array, max_count)     \
  do {                                                \
    for (int i = 0; i < max_count; i++) {             \
      array[i].isAlive = false;                       \
    }                                                 \
