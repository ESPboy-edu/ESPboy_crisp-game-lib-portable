...
static char PROGMEM *title = "PAKU PAKU";
...
static void update() {
  ...
  FOR_EACH_PTR(dots, PAKU_PAKU_MAX_DOT_COUNT, i) {
  ...
  COUNT_IS_ALIVE_PTR(dots, PAKU_PAKU_MAX_DOT_COUNT, dc);
  if (dc == 0) {
    play(COIN);
    addDots();
  }
