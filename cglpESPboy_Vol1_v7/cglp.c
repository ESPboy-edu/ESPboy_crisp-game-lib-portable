...
static Collision globalHitCollision;
...
float tempo = 120.0f * (60.0f / (float)FPS);
...
#define MAX_HIT_BOX_COUNT 150//256
/// \endcond
...
// Drawing
/// \cond
#define MAX_DRAWING_HIT_BOXES_COUNT 50//64
/// \endcond
//static HitBox drawingHitBoxes[MAX_DRAWING_HIT_BOXES_COUNT];
static HitBox *drawingHitBoxes;
...
Collision *rect(float x, float y, float w, float h) {
  initCollision(&globalHitCollision);
  beginAddingRects();
  addRect(false, x, y, w, h, &globalHitCollision);
  endAddingRects();
  return &globalHitCollision;
}
...
Collision *box(float x, float y, float w, float h) {
  initCollision(&globalHitCollision);
  beginAddingRects();
  addRect(true, x, y, w, h, &globalHitCollision);
  endAddingRects();
  return &globalHitCollision;
}
...
Collision *line(float x1, float y1, float x2, float y2) {
  initCollision(&globalHitCollision);
  beginAddingRects();
  drawLine(x1, y1, x2 - x1, y2 - y1, &globalHitCollision);
  endAddingRects();
  return &globalHitCollision;
}
...
Collision *bar(float x, float y, float length, float angle) {
  ...
  beginAddingRects();
  drawLine(p.x, p.y, l.x, l.y, &globalHitCollision);
  endAddingRects();
  return &globalHitCollision;
}
...
Collision *arc(float centerX, float centerY, float radius, float angleFrom,
  ...
  for (int i = 0; i < lc; i++) {
    ...
    ox = p2x - p1x;
    oy = p2y - p1y;
    drawLine(p1x, p1y, ox, oy, &globalHitCollision);
    p1x = p2x;
    p1y = p2y;
  }
  endAddingRects();
...
static void setColorGrid(
    unsigned char grid[CHARACTER_HEIGHT][CHARACTER_WIDTH + 1],
    unsigned char colorGrid[CHARACTER_HEIGHT][CHARACTER_WIDTH],
    int8_t colorIndex, HitBox *hb) {
  Vector leftTop;
  ...
  for (int y = 0; y < CHARACTER_HEIGHT; y++) {
    for (int x = 0; x < CHARACTER_WIDTH; x++) {
      ...
      if (ci == NULL) {
        ...
        if (colorIndex == BLACK) 
           colorGrid[y][x] = ci - colorGridChars;
        else
...
static void drawCharacter(int index, float x, float y, bool _hasCollision, bool isText, Collision *hitCollision) {
  if ((isText && (index < '!' || index > '~')) || (!isText && (index < 'a' || index > 'z'))) {
    return;
  }

  bool shouldCache = (!isText) || 
                     (index >= '0' && index <= '9') || 
                     index == 'H' || index == 'I' || 
                     index == '+' || index == '-' || index == 'x';

  CharacterPattern *cp_ptr;
  CharacterPattern temp_cp; // Временный буфер на стеке для "одноразовых" букв

  if (shouldCache) {
    uint32_t hash = (uint32_t)index | 
                    ((uint32_t)isText << 8) | 
                    ((uint32_t)color << 9) | 
                    ((uint32_t)characterOptions.isMirrorX << 13) | 
                    ((uint32_t)characterOptions.isMirrorY << 14) | 
                    ((uint32_t)characterOptions.rotation << 15);

    int slot = hash % CACHE_SIZE;

    if (spriteCache[slot].hash == hash) {
      cp_ptr = &spriteCache[slot].cp;
    } else {
      cp_ptr = &spriteCache[slot].cp;
      setColorGrid(isText ? textPatterns[index - '!'] : characters[index - 'a'], cp_ptr->grid, color, &cp_ptr->hitBox);
      spriteCache[slot].hash = hash;
    }
  } else {
    cp_ptr = &temp_cp;
    setColorGrid(textPatterns[index - '!'], cp_ptr->grid, color, &cp_ptr->hitBox);
  }

  if (color != TRANSPARENT && x > -CHARACTER_WIDTH && x < viewSizeX &&
      y > -CHARACTER_HEIGHT && y < viewSizeY) {
    md_drawCharacter(&cp_ptr->grid, x, y);
  }

  if (hasCollision && _hasCollision) {
    HitBox *thb = &cp_ptr->hitBox;
    HitBox hb;
...
static Collision *drawCharacters(char *msg, float x, float y, bool _hasCollision, bool isText) {
  initCollision(&globalHitCollision);
  int ml = strlen(msg);
  x -= CHARACTER_WIDTH / 2;
  y -= CHARACTER_HEIGHT / 2;
  for (int i = 0; i < ml; i++) {
    drawCharacter(msg[i], x, y, _hasCollision, isText, &globalHitCollision);
    x += 6;
  }
  return &globalHitCollision;
}
...
#define MAX_SCORE_BOARD_COUNT 11//16
/// \endcond
//static ScoreBoard scoreBoards[MAX_SCORE_BOARD_COUNT];
static ScoreBoard *scoreBoards;
...
/// \endcond
static char descriptions[MAX_DESCRIPTION_LINE_COUNT][MAX_DESCRIPTION_STRLEN + 1];
...
static void drawGameOver() {
  ...
  if (isNewRecord) {
    char *recordText = "NEW RECORD!";
    color = (ticks / 30) % 2 == 0 ? RED : YELLOW; 
    drawCharacters(recordText,
                   (viewSizeX - strlen(recordText) * CHARACTER_WIDTH) / 2,
                   viewSizeY * 0.5f + 12, false, true);
  }
}
...
static void initGameOver() {
  ...
  if (isReplaying) {
    ...
    if (prevScore > hiScore) {
      hiScore = prevScore;
      md_saveHighScore(currentGameIndex, hiScore);
      isNewRecord = true;
    }
  }
  gameOverTicks = 0;
...
static void resetGame(int gameIndex) {
  ...
  if (spriteCache) {
      memset(spriteCache, 0, sizeof(SpriteCache) * CACHE_SIZE);
  }

  initScore();
  hiScore = highScores[gameIndex];
  ...
  if (options.isDarkColor)
     md_clearScreen(BLACK);
  else
     md_clearScreen(LIGHT_BLACK);
...
uint8_t *sharedGameRAM;


void allocateArrays(){
  hitBoxes = (HitBox*)malloc(sizeof(HitBox) * MAX_HIT_BOX_COUNT);
  drawingHitBoxes = (HitBox*)malloc(sizeof(HitBox) * MAX_DRAWING_HIT_BOXES_COUNT);
  scoreBoards = (ScoreBoard*)malloc(sizeof(ScoreBoard) * MAX_SCORE_BOARD_COUNT);  
  recordedInputs = (uint8_t*)malloc(sizeof(uint8_t) * MAX_RECORDED_INPUT_COUNT);
  allocateParticleMemory();
  allocateOscilMemory();
  sharedGameRAM = (uint8_t*)malloc(8000);
  spriteCache = (SpriteCache*)malloc(sizeof(SpriteCache) * CACHE_SIZE);
  if (spriteCache) {memset(spriteCache, 0, sizeof(SpriteCache) * CACHE_SIZE);}
}
