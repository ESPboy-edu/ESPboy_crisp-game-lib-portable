...
static void update() {
  if (!ticks) {
    tanks = (Tank *)sharedGameRAM;
    bullets = (Bullet *)(sharedGameRAM + (sizeof(Tank) * REFLECTOR_MAX_TANK_COUNT));
    explosions = (Explosion *)(sharedGameRAM + (sizeof(Tank) * REFLECTOR_MAX_TANK_COUNT) + (sizeof(Bullet) * REFLECTOR_MAX_BULLET_COUNT));
    vectorSet(&ufo.pos, 40, 9);
    ufo.vx = 1;
    ufo.angle = 0;
    ufo.power = 0;
    INIT_UNALIVED_ARRAY_PTR(tanks, REFLECTOR_MAX_TANK_COUNT);
    tankIndex = 0;
    INIT_UNALIVED_ARRAY_PTR(bullets, REFLECTOR_MAX_BULLET_COUNT);
    bulletIndex = 0;
    INIT_UNALIVED_ARRAY_PTR(explosions, REFLECTOR_MAX_EXPLOSION_COUNT);
    explosionIndex = 0;
    multiplier = 1;
  }
  color = LIGHT_BLACK;
  ...
  thickness = 3;
  COUNT_IS_ALIVE_PTR(tanks, REFLECTOR_MAX_TANK_COUNT, tc);
  if (tc == 0) {
    nextTankTicks = 0;
  }
  nextTankTicks--;
  ...
  color = LIGHT_RED;
  COUNT_IS_ALIVE_PTR(explosions, REFLECTOR_MAX_EXPLOSION_COUNT, ec);
  if (ec == 0) {
    multiplier = 1;
  }
  FOR_EACH_PTR(explosions, REFLECTOR_MAX_EXPLOSION_COUNT, i) {
  ...
  FOR_EACH_PTR(tanks, REFLECTOR_MAX_TANK_COUNT,i) {
  ...
  thickness = 3;
  barCenterPosRatio = 0.5;
  FOR_EACH_PTR(bullets, REFLECTOR_MAX_BULLET_COUNT, i) {
